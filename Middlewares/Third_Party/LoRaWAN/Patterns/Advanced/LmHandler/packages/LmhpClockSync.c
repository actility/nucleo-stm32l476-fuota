 ...
 *
 * \param [IN] mlmeConfirm     MLME confirm primitive data
 */
static void LmhpClockSyncOnMlmeConfirm( MlmeConfirm_t *mlmeConfirm );
...
static LmhPackage_t LmhpClockSyncPackage =
{
    ...
    .OnMlmeConfirmProcess = LmhpClockSyncOnMlmeConfirm,        // Process info about DevTimeAns
...
static bool isDeviceTimeAnsReceived;

static void LmhpClockSyncOnMlmeConfirm( MlmeConfirm_t *mlmeConfirm )
{
    // Check for DevTimeAns and set flag that it was received
    if( ( mlmeConfirm->MlmeRequest == MLME_DEVICE_TIME ) && ( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK ) )
    {
        isDeviceTimeAnsReceived = true;
    }
}
    ...
    while( cmdIndex < mcpsIndication->BufferSize )
    {
        switch( mcpsIndication->Buffer[cmdIndex++] )
        {
            ...
            case CLOCK_SYNC_APP_TIME_ANS:
            {
                ...
                // mcpsIdication is set only if DevTimeAns MAC command was received with data 
                if( ( mcpsIndication->DeviceTimeAnsReceived == true ) || ( isDeviceTimeAnsReceived == true ) )
                {
                    break;
                }
                ...
                if( ( mcpsIndication->Buffer[cmdIndex++] & 0x0F ) == LmhpClockSyncState.TimeReqParam.Fields.TokenReq )
                {
                    ...
                    SysTimeSet( curTime );
                    PRINTF("timeCorrection: %d, before time: %u corrected time: %u\r\n",  timeCorrection, sec, curTime.Seconds); 
...
LmHandlerErrorStatus_t LmhpClockSyncAppTimeReq( void )
{
    ...
    // Substract Unix to Gps epcoh offset. The system time is based on Unix time.
    curTime.Seconds -= UNIX_GPS_EPOCH_OFFSET;
    PRINTF("devtime UNIX: %u, devtime GPS: %u\r\n",  sec, curTime.Seconds); 
