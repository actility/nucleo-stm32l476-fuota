...
static void McpsIndication( McpsIndication_t *mcpsIndication )
{
    ...
    PRINTF("McpsIndication status %d\n\r", mcpsIndication->Status);
...
static void MlmeConfirm( MlmeConfirm_t *mlmeConfirm )
{
    ...
    case MLME_DEVICE_TIME:
        {
            PRINTF("MLME_DEVICE_TIME received\r\n");
            if( mlmeConfirm->Status == LORAMAC_EVENT_INFO_STATUS_OK )
            {
                PRINTF("Time updated ok\r\n");
                // Show upper later that MAC layer DevTimeAns was received
                // Should stop asking for time
                LmHandlerCallbacks->OnSysTimeUpdate( ); 
            }
...
static void MlmeIndication( MlmeIndication_t *mlmeIndication )
{
    ...
    PRINTF("MLME Indication %d\n\r", mlmeIndication->MlmeIndication);
...
LmHandlerErrorStatus_t LmHandlerPackageRegister( uint8_t id, void *params )
{
    ...
    switch( id )
    {
        ...
        case PACKAGE_ID_FWMANAGEMENT:
        {
            package = LmhpFWManagementPackageFactory( );
            break;
        }
    }
    if( package != NULL )
